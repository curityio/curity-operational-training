IF OBJECT_ID('sp_clear_tokens', 'P') IS NOT NULL
    DROP PROCEDURE sp_clear_tokens;
GO

CREATE PROCEDURE sp_clear_tokens
AS
...
BEGIN
    ...
    PRINT CONCAT(
        'sp_clear_tokens: Starting batch deletion at ',
        CONVERT(varchar(23), SYSDATETIME(), 121)
    );

    BEGIN TRY
        BEGIN TRANSACTION;
        DELETE TOP (@batch_size) FROM idsvr.dbo.tokens WHERE expires < DATEDIFF(s, '1970-01-01 00:00:00', CURRENT_TIMESTAMP);
        COMMIT TRANSACTION;

        SET @rows = @@ROWCOUNT;
        SET @deleted += @rows;
        PRINT CONCAT(
            'sp_clear_tokens: Completed batch deletion at ', 
            CONVERT(varchar(23), SYSDATETIME(), 121),
