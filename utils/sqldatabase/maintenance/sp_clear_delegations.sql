IF OBJECT_ID('sp_clear_delegations', 'P') IS NOT NULL
    DROP PROCEDURE sp_clear_delegations;
GO

CREATE PROCEDURE sp_clear_delegations
AS
...
BEGIN
    ...
    PRINT CONCAT(
        'sp_clear_delegations: Starting batch deletion at ', 
        CONVERT(varchar(23), SYSDATETIME(), 121)
    );

    BEGIN TRY
        BEGIN TRANSACTION;
        DELETE TOP (@batch_size) FROM idsvr.dbo.delegations WHERE expires < DATEDIFF(s, '1970-01-01 00:00:00', CURRENT_TIMESTAMP);
        COMMIT TRANSACTION;

        SET @rows = @@ROWCOUNT;
        SET @deleted += @rows;
        PRINT CONCAT(
            'sp_clear_delegations: Completed batch deletion at ', 
            CONVERT(varchar(23), SYSDATETIME(), 121),
